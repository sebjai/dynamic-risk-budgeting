...
class betaNet(nn.Module):
    ...
    def forward(self, h, Y):
        ...
        beta = self.softmax(self.hidden_to_out(x)).squeeze()
...
class DynamicRiskParity():
    ...
    def __init__(self, Simulator : Simulator_OU, X0=1, B = 0, alpha=0.8, p=0.5):
        ...
        if B == 0:
            ...
            self.B_scaled = 1.0*self.B.to(self.device)
            self.B_scaled[0,:] *= 0.7
            self.B_scaled[1,:] *= 1.2
            
            # self.B_scaled[3,:] *= 0.12/0.1
            # self.B_scaled[4,:] *= 0.15/0.1
        else:
        ...
        for t in range(self.T):
            ...
            self.beta.append(betaNet(nIn=self.d+2,
                                nOut=self.d,
                                ...
                                device=self.device))
            self.beta_optimizer.append(optim.AdamW(self.beta[-1].parameters(), 
                                              lr = 0.001))
    ...
    def __initialize_CVaR_VaR_Mean_Net__(self, device='cpu'):
        ...
        gru_total_hidden = self.beta[0].gru.num_layers*self.beta[0].gru.hidden_size
    ...
    def __RunEpoch__(self, batch_size = 256):
        ...
        h = torch.zeros((self.T+1,
                         self.beta[0].gru.num_layers, 
                         batch_size, 
                         self.beta[0].gru.hidden_size)).to(self.device)
        ...
        h[1,...], beta[0,:,:] = self.beta[0](h[0,...], Y[0,...])
        ...
        for t in range(1, self.T):
            ...
            h[t+1,...], beta[t,:,:] = self.beta[t](h[t,...], Y[t,...])
            ...
            for t in t_rnd:
                for i in i_rnd:
                    ...
                    loss = torch.mean(theta[t,:,i] * (-Delta_S[t,:,i]) * cumprod_Gamma[t,:].detach())\
                        - self.eta*torch.mean( self.B_scaled[t,:,i] * theta[t,:,i]/theta[t,:,i].detach())
                    
                    self.beta_optimizer[t].zero_grad()
                    self.wealth_0_optimizer.zero_grad()    
                    
                    loss.backward()
                    
                    self.beta_optimizer[t].step()
                    self.wealth_0_optimizer.step()
                    
            beta_loss  = V[0,0] - self.eta*torch.sum(torch.mean( self.B_scaled * torch.log(theta), axis=1))
    ...
    def PlotSummary(self):
        ...
        for k in range(self.d):
            for j in range(self.T):
                ...
                plt.ylim(0,self.B[0,0,0].detach().cpu().numpy()*self.eta*2)
