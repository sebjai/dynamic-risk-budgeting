...
class  Simulator_OU():
    ...
    def __init__(self, n_assets = 3, T = 12, steps_per_period=1, dt=1/12):
    ...
    def Simulate(self, Nsims=256):
        ...
        for i in range(self.T):
            ...
            for j in range(self.m):
                ...
                X[:, i+1, :] *= np.exp(-self.kappa*self.dt) 
                X[:, i+1, :] += self.sigma * dW
                
        t = self.m*self.dt*np.linspace(1e-10, self.T, self.T+1).reshape(1,-1, 1)
