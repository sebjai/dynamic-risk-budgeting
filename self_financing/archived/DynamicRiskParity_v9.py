...
class Net(nn.Module):
    ...
    def forward(self, h, Y):
        ...
        h = torch.cat((h.flatten(start_dim=-3), 
                       Y_flat), 
                      axis=-1)
...
class betaNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.gru_to_hidden = nn.Linear(gru_hidden*gru_layers+3+nOut, linear_hidden).to(self.device)
    ...
    def forward(self, h, Y):
        ...
        beta = self.softplus(self.hidden_to_out(x)).squeeze()
...
class DynamicRiskParity():
    ...
    def __init__(self, Simulator : Simulator_OU, X0=1, B = 0, alpha=0.8, p=0.5):
        ...
        for t in range(self.T):
            ...
            for i in range(self.d):
                beta.append(betaNet(nIn=self.d+2,
                                    nOut=1,
                                    ...
                                    device=self.device))
                beta_optimizer.append(optim.AdamW(beta[-1].parameters(), 
                                                  lr = 0.001))
            self.beta.append(beta)
            self.beta_optimizer.append(beta_optimizer)

        self.beta = np.array(self.beta)
        self.beta_optimizer = np.array(self.beta_optimizer)
    ...
    def __initialize_CVaR_VaR_Mean_Net__(self, device='cpu'):
        ...
        gru_total_hidden = self.beta[0,0].gru.num_layers*self.beta[0,0].gru.hidden_size
        n_in = gru_total_hidden*self.d+2+self.Simulator.n
    ...
    def __F_Score__(self, h, Y, X):
        ...
        F = self.F(h.unsqueeze(axis=0).repeat(N,1,1,1,1), Z)
    ...
    def __RunEpoch__(self, batch_size = 256):
        ...
        h = torch.zeros((self.T+1,
                         self.d,
                         self.beta[0,0].gru.num_layers, 
                         batch_size, 
                         self.beta[0,0].gru.hidden_size)).to(self.device)
        ...
        for i in range(self.d):
            h[1,i,...], beta[0,:,i] = self.beta[0,i](h[0,i,...], Y[0,...])
        ...
        for t in range(1, self.T):
            ...
            wealth[t,:] = torch.sum( theta[t-1,:,:] * S[t,:,:], axis=1).clone()
            ...
            for i in range(self.d):
                h[t+1,i,...], beta[t,:,i] = self.beta[t,i](h[t,i,...], Y[t,...])
                
            # rebalance assets holdings
            theta[t,:,:] = beta[t,:,:].clone() * wealth[t,:].reshape(-1,1).clone() / S[t,:,:]  
    ...
    def Update_ValueFunction(self, N_iter  = 100, batch_size = 256, n_print=100):
        ...
        for j in range(N_iter):
            ...
            for i in range(0, self.T):
                ...
                if i < self.T-1:
                    cost_plus_V_one_ahead += self.risk_measure_target(h[i+2,...].transpose(0,2), 
                                                                      Y[i+1,...])
                
                loss += self.__Score__(self.VaR(h[i+1,...].transpose(0,2), Y[i,...]),
                                       self.CVaR(h[i+1,...].transpose(0,2), Y[i,...]),
                                       self.mu(h[i+1,...].transpose(0,2), Y[i,...]),
    ...
    def Update_F(self, N_iter  = 100, batch_size = 256, n_print=100):
        ...
        for j in range(N_iter):
            ...
            for i in range(0, self.T):
                ...
                loss += self.__F_Score__(h[i+1,...].transpose(0,2), 
                                         Y[i,...], 
                                         cost_plus_V_one_ahead)
    ...
    def __Compute_V_Gamma__(self, costs, h, Y):
            ...
            for j in range(self.T):
                V[j,:] = self.risk_measure_target(h = h[j+1,...].transpose(0,2), 
                                                  Y=Y[j,...]).squeeze()
            ...
            for j in range(self.T):
                U[j,:] = self.F(h[j+1,...].transpose(0,2), Z[j,...])[:,0]
        ...
        for k in range(N_iter):
            ...
            for t in range(self.T):
                for i in range(self.d):
                    ...
                    loss = torch.mean(theta[t,:,i] * (-Delta_S[t,:,i]) * cumprod_Gamma[t,:].detach())\
                        - self.eta*torch.mean( self.B[t,:,i] * theta[t,:,i]/theta[t,:,i].detach())
                    
                    self.beta_optimizer[t,i].zero_grad()
                    self.wealth_0_optimizer.zero_grad()    
                    
                    loss.backward(retain_graph=True)
                    
                    self.beta_optimizer[t,i].step()
                    self.wealth_0_optimizer.step()
                    
            beta_loss  = V[0,0] - self.eta*torch.sum(torch.mean( self.B * torch.log(theta), axis=1))
    ...
    def Train(self, n_iter=10_000, n_print = 10, M_value_iter = 10, M_policy_iter=1, batch_size=256):
        ...
        for i in tqdm(range(n_iter)):
            ...
            # this udpates F
            self.Update_F(N_iter=2, n_print=500, batch_size=batch_size)
