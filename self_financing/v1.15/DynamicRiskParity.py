...
class ThetaNet(nn.Module):
    ...
    def forward(self, h, Y):
        ...
        # theta = 5*self.sigmoid(self.hidden_to_out(x)).squeeze()
...
class DynamicRiskParity():
    ...
    def __init__(self, Simulator : Simulator_OU, X0=1, B = 0, alpha=0.8, p=0.5):
        ...
        for t in range(self.T):
            ...
            self.theta.append(ThetaNet(nIn=self.d+2,
                                ...
                                linear_layers=5,
                                device=self.device))
            self.theta_optimizer.append(optim.AdamW(self.theta[-1].parameters(), 
                                              lr = 0.001))
    ...
    def __initialize_CVaR_VaR_Mean_Net__(self, device='cpu'):
        ...
        for t in range(self.T):
            ...
            self.F_optimizer.append(optim.AdamW(self.F[-1].parameters(), lr=0.001))
        ...
        for t in range(self.T):
            ...
            self.mu_target.append(copy.deepcopy(self.mu[-1]))
            self.mu_optimizer.append(optim.AdamW(self.mu[-1].parameters(), lr=0.001))
        ...
        for t in range(self.T):
            ...
            self.psi_target.append(copy.deepcopy(self.psi[-1]))
            self.psi_optimizer.append(optim.AdamW(self.psi[-1].parameters(), lr=0.001))
        ...
        for t in range(self.T):
            ...
            self.chi_optimizer.append(optim.AdamW(self.chi[-1].parameters(), lr=0.001))
            self.chi_target.append(copy.deepcopy(self.chi[-1]))
    ...
    def __RunEpoch__(self, batch_size = 256):
        ...
        theta_hat[0,:,:] = theta[0,:,:].clone() # / torch.sum( theta[0,:,:] * S[0,:,:], axis=1).clone().detach().reshape(-1,1)
    
        # stores wealth process
        wealth = torch.zeros((self.T+1, batch_size)).to(self.device)
        wealth[0,:] = torch.sum(theta_hat[0,:,:] * S[0,:,:], axis=1)
            
        for t in range(1, self.T):
            
            # new wealth at period end
            wealth[t,:] = torch.sum( theta_hat[t-1,:,:] * S[t,:,:], axis=1).clone()
            
            # concatenate t_i, and asset prices    
            Y[t,...] = torch.cat(((t*self.dt)*ones, 
                                  wealth[t,:].reshape(-1,1).clone(),
                                  S[t,:,:]), axis=1).unsqueeze(axis=1)
            
            # push through the neural-net to get new number of assets
            h[t+1,...], theta[t,:,:] = self.theta[t](h[t,...], Y[t,...])
            
            weight = wealth[t,:].clone() / torch.sum( theta[t,:,:] * S[t,:,:], axis=1).clone()
            
            theta_hat[t,:,:] = theta[t,:,:].clone() * weight.reshape(-1,1).detach()
                
        wealth[-1,:] = torch.sum( theta_hat[-1,:,:] * S[-1,:,:], axis=1).clone()
            
        costs = -torch.diff(wealth, axis=0)   
        
        beta = theta_hat * S[:-1,...] / torch.sum(theta_hat * S[:-1,...], axis=2).unsqueeze(axis=2)
        
        return costs, h, Y, beta, theta, theta_hat, S, wealth    
    ...
    def Update_ValueFunction(self, N_iter  = 100, batch_size = 256, n_print=100):
        ...
        for j in range(N_iter):
            ...
            for t in range(0, self.T):
                ...
                cost_plus_V_one_ahead = costs[t,:]
                
                if t < self.T-1:
                    cost_plus_V_one_ahead += self.risk_measure_target(t,
                                                                      h[t+2,...].transpose(0,1), 
                                                                      Y[t+1,...])
                
                loss = self.__V_Score__(self.VaR(t, h[t+1,...].transpose(0,1), Y[t,...]),
                                       self.CVaR(t, h[t+1,...].transpose(0,1), Y[t,...]),
                                       self.mu[t](h[t+1,...].transpose(0,1), Y[t,...]),
        ...
        for t in range(0, self.T):                
            self.chi_target[t] = copy.deepcopy(self.chi[t])
            self.psi_target[t] = copy.deepcopy(self.psi[t])
            self.mu_target[t] = copy.deepcopy(self.mu[t])
    ...
    def __Compute_V_Gamma__(self, costs, h, Y):
            ...
            for t in range(self.T):
                U[t,:] = self.F[t](h[t+1,...].transpose(0,1),
                                   Z[t,...])[:,0]
    ...
    def RiskContributions(self, batch_size=2048):
        ...
        for t in range(self.T):
            ...
            for n in range(self.d):
                ...
                Z = theta_hat[t,:,n]  * (-Delta_S[t,:,n])  * cumprod_Gamma[t,:]
        ...
        for k in range(N_iter):
            ...
            for t in t_rnd:
                ...
                for i in i_rnd:
                    ...
                    loss += torch.mean(theta_hat[t,:,i] * (-Delta_S[t,:,i]) * cumprod_Gamma[t,:].detach())\
                        - self.eta*torch.mean( self.B[t,:,i] * torch.log(theta_hat[t,:,i]))
    ...
    def Train(self, n_iter=10_000, n_print = 10, M_value_iter = 10, M_policy_iter=1, batch_size=256, name=""):
        ...
        for i in tqdm(range(n_iter)):
            ...
            # this udpates F
            self.Update_F(N_iter=50, n_print=500, batch_size=batch_size)
