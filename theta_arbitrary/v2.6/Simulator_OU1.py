...
class  Simulator_OU():
    ...
    def __init__(self, n_assets = 3, T = 12, dt=1/12):
        ...
        self.sigma = np.array([[0.2, 0.3],[0.4, 0.3]])
    ...
    def Simulate(self, Nsims=256):
        ...
        for t in range(self.T):
            ...
            X[:, t+1, :] = X[:,t,:] * np.exp(-self.kappa*self.dt) + self.sigma[t] * dW
            
        t = self.dt*np.linspace(1e-10, self.T, self.T+1).reshape(1,-1, 1)
        g = self.mu.reshape(1,1,-1) * t - self.sigma[0,:].reshape(1,1,-1)**2*(1-np.exp(-2*self.kappa.reshape(1,1,-1)*t))/(4.0*self.kappa.reshape(1,1,-1))        
