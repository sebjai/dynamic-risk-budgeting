...
class net(nn.Module):
    ...
    def __init__(self, 
        ...
        self.in_to_hidden = nn.Linear(nIn, linear_hidden).to(self.device)
        self.hidden_to_hidden = nn.ModuleList([nn.Linear(linear_hidden, linear_hidden).to(self.device) for i in range(linear_layers-1)])
...
class dynamic_risk_budget():
    ...
    def __init__(self, env : heston, X0=1, B = 0, alpha=0.8, p=0.5, name=""):
    ...
    def __initialize_nets__(self, device='cpu'):
        ...
        self.F = net(nIn=self.d+2, nOut=1,
                     linear_hidden=32, linear_layers=8,
                     out_activation='sigmoid', device=self.device)
        # the in to hidden has to take in one extra feature = z
        self.F.in_to_hidden = nn.Linear(self.d+3,
    ...
    def __run_epoch__(self, batch_size = 256):
        ...
        for t in range(1, self.T):
            ...
            theta[t,:,:] = self.theta[t](Y[t,...].detach())
        ...
        return costs, Y, beta, theta, var_theta, w, X, wealth
    ...
    def get_risk_measure(self, Y, target = False):
        ...
        for t in range(self.T):
            ...
            if not target:
                cond_RM = self.cond_RM(Y[t,...])
            else:
                cond_RM = self.cond_RM_target(Y[t,...])
    ...
    def __update_risktogo__(self, N_iter  = 100, batch_size = 256, n_print=100):
        ...
        for j in range(N_iter):
            ...
            costs, Y, beta, theta, var_theta, w, X, wealth = self.__run_epoch__(batch_size)
    ...
    def __get_v_gamma__(self, theta, dX, w, Y):
        ...
        for t in range(self.T):
            ...
            u = self.F(Y[t,...], A.unsqueeze(axis=-1))
            U[t,:] = u.squeeze()
    ...
    def __get_gateaux_terms__(self, batch_size=2048):
        ...
        costs, Y, beta, theta_grad, var_theta, w, X, wealth = self.__run_epoch__(batch_size)
    ...
    def plot_summary(self):
        ...
        plt.savefig(self.name + '_' + 'sumRC_V.pdf', format='pdf')
        plt.show()      
        ...
        plt.suptitle(r'$RC$')
        plt.tight_layout()
        plt.savefig(self.name + '_RC.pdf', format='pdf')
        plt.show()      
                
       
    def plot_paths(self, batch_size=1, title=None):
        
        costs, Y, beta, theta, var_theta, w, S, wealth = self.__run_epoch__(batch_size)
        ...
        for j in range(1, self.T):
            ...
            plt.savefig(self.name + '_beta-' + str(j) + '.pdf', format='pdf')
            plt.show()
