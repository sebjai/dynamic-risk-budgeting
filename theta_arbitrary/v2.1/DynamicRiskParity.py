...
class DynamicRiskParity():
    ...
    def __initialize_CVaR_VaR_Mean_Net__(self, device='cpu'):
        ...
        # for conditional cdf of c_t + V_{t+1} without the self-financing rescaling of c_t
        self.G = Net(n_in=n_in+1, n_out=1, nNodes=16, nLayers=3, out_activation='sigmoid', device=self.device)
        self.G_optimizer = optim.AdamW(self.G.parameters(), lr=0.001)
    ...
    def __F_Score__(self, h, Y, X):
        ...
        return self.__cdf_Score__(h,Y,X, self.F)
    
    def __G_Score__(self, h, Y, X):
        
        return self.__cdf_Score__(h,Y,X, self.G)    
    
    def __cdf_Score__(self, h, Y, X, cdf):
        ...
        F = cdf(h.unsqueeze(axis=0).repeat(N,1,1,1), Z)
    ...
    def __update_F__(self, N_iter  = 100, batch_size = 256, n_print=100):
        ...
        for j in range(N_iter):
            ...
            def loss(c, score_func, optim, sched):
                ...
                for t in range(0, self.T):
                    ...
                    cost_plus_V_one_ahead = c[t,:]
                    
                    if t < self.T-1:
                        cost_plus_V_one_ahead += self.risk_measure_target(h[t+1,...].transpose(0,1), 
                                                                          Y[t+1,...])
                    
                    loss += score_func(h[t,...].transpose(0,1),
                                       Y[t,...],
                                       cost_plus_V_one_ahead.detach())                
            ...
            c = torch.sum(theta_hat.detach() * (X[:-1,:,:]-X[1:,:,:]), axis=2)
            loss_F = loss(costs, self.__F_Score__, self.F_optimizer, self.F_scheduler)
            
            # this cost does not have the rescaling of theta to theta_hat
            c = torch.sum(theta.detach() * (X[:-1,:,:]-X[1:,:,:]), axis=2)
            loss_G = loss(c, self.__G_Score__, self.G_optimizer, self.G_scheduler)
            
            self.F_loss.append(loss_F.item() + loss_G.item())
            
            
    def __get_v_gamma__(self, c, c_hat, h, Y):
            ...
            c_hat_plus_V_onestep_ahead  = (c_hat + V[1:,:])
            c_plus_V_onestep_ahead  = (c + V[1:,:])
            

            def generate_gamma(c_V, cdf):
                U = torch.zeros((self.T, batch_size)).to(self.device)
                
                Z = torch.concat((Y,
                                  c_V.unsqueeze(axis=2).unsqueeze(axis=3)),
                                 axis=3)
                
                for t in range(self.T):
                    U[t,:] = cdf(h[t,...].transpose(0,1),
                                 Z[t,...])[:,0]
                    
                return self.gamma(U)
                
            Gamma_hat = generate_gamma(c_hat_plus_V_onestep_ahead, self.F)
            Gamma = generate_gamma(c_plus_V_onestep_ahead, self.G)
            
            return V, Gamma, Gamma_hat
        
    def __get_gateaux_terms__(self, batch_size=2048):
        
        costs, h, Y, beta, theta_grad, theta_hat, X, wealth = self.__RunEpoch__(batch_size)
        
        c = torch.sum(theta_grad.detach() * (X[:-1,:,:]-X[1:,:,:]), axis=2)
        c_hat = torch.sum(theta_hat.detach() * (X[:-1,:,:]-X[1:,:,:]), axis=2)
        
        V,  Gamma, Gamma_hat = self.__get_v_gamma__(c.detach(), 
                                                    c_hat.detach(),
                                                    h.detach(),
                                                    Y.detach())
        ...
        for t in range(self.T-1): 
            ...
            for i in range(self.d):
                ...
                Z[t,:,i] = Delta_X[t,:,i] * Gamma[t,:] \
                    + X[t+1,:,i] * theta_tp1_dX_tp1/theta_tp1_X_tp1 * Gamma[t,:] * Gamma_hat[t+1,:]
                    
        Z[-1,:,i] = Delta_X[t,:,i] * Gamma[t,:]
    ...
    def __update_policy__(self, N_iter=100, batch_size = 256):
        ...
        for k in range(N_iter):
            ...
            # loss = torch.mean(Z) - self.eta*torch.mean( self.B * theta_grad/theta_grad.detach())
    ...
    def PlotHist(self, batch_size = 10_000):
        ...
        V,  Gamma = self.__get_v_gamma__(costs, h, Y)
