...
class DynamicRiskParity():
    ...
    def __run_epoch__(self, batch_size = 256):
        ...
        for t in range(1, self.T):
            ...
            w[t-1,:] = torch.sum(theta[t-1,:,:].detach() * X[t,:,:], axis=1) / torch.sum(theta[t,:,:].detach() * X[t,:,:], axis=1) 
    ...
    def __get_gateaux_terms__(self, batch_size=2048):
        ...
        V,  Gamma = self.__get_v_gamma__(theta.detach(),
                                         ...
                                         h.detach(),
                                         Y.detach())
        theta_X = torch.sum(theta * X[:-1,...], axis=2)
        ...
        for t in range(self.T):
            ...
            if t < self.T-1:
                V_tp1 = self.risk_measure_target(h[t+1,...].transpose(0,1), 
                                                 Y[t+1,...])
                Z[t,:,:] += (X[t+1,:,:]/theta_X[t+1,:].reshape(-1,1)) * V_tp1
            ...
            for t in t_rnd:
                ...
                for i in i_rnd:
                    ...
                    loss = torch.mean(Z[t,:,i]) \
                        - self.eta*torch.mean( self.B[t,:,i] * theta_grad[t,:,i]/theta_grad[t,:,i].detach())
    ...
    def PlotHist(self, batch_size = 10_000):
        ...
        kde_bins = np.linspace(-1, 1, 501)
        
        V_bins=np.linspace(-1, 1, 51)
        V_kde_bins = np.linspace(-1, 1, 501)
