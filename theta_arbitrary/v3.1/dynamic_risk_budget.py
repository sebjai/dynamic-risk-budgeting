...
class dynamic_risk_budget():
    ...
    def __initialize_nets__(self, device='cpu'):
        ...
        self.mu = net(nIn=self.d+2, nOut=1,
                      gru_hidden=self.env.n, gru_layers=5,
                      linear_hidden=32, linear_layers=5,
                      device=self.device)
        self.mu_target = copy.deepcopy(self.mu)
        self.mu_optimizer, self.mu_scheduler = self.__get_optim_sched__(self.mu)
        
        # for conditional VaR
        self.psi = net(nIn=self.d+2, nOut=1,
                       gru_hidden=self.env.n, gru_layers=5,
                       linear_hidden=32, linear_layers=5,
                       device=self.device)
        self.psi.gru = self.mu.gru
        self.psi_target = copy.deepcopy(self.psi)
        self.psi_optimizer, self.psi_scheduler = self.__get_optim_sched__(self.psi)
        
        
        
        # for increment from conditional VaR to conditional CVaR
        # i.e., CVaR = chi + psi
        self.chi = net(nIn=self.d+2, nOut=1,
                       gru_hidden=self.env.n, gru_layers=5,
                       linear_hidden=32, linear_layers=5,
                       out_activation='softplus', device=self.device)
        self.chi.gru = self.mu.gru
        self.chi_target = copy.deepcopy(self.chi)
        self.chi_optimizer, self.chi_scheduler = self.__get_optim_sched__(self.chi)
    ...
    def get_risk_measure(self, Y, target = False):
        ...
        h_mu = torch.zeros((self.T+1,
                         self.mu.gru.num_layers, 
                         Y.shape[1], 
                         self.mu.gru.hidden_size)).to(self.device)
        h_psi = h_mu.clone()
        h_chi = h_mu.clone()
        ...
        for t in range(self.T):
            ...
            if not target:
                h_mu[t+1,...], mu[t,:] = self.mu(h_mu[t,...], Y[t,...])
                h_psi[t+1,...], psi[t,:] = self.psi(h_psi[t,...], Y[t,...])
                h_chi[t+1,...], chi[t,:] = self.chi(h_chi[t,...], Y[t,...])
            else:
                h_mu[t+1,...], mu[t,:] = self.mu_target(h_mu[t,...], Y[t,...])
                h_psi[t+1,...], psi[t,:] = self.psi_target(h_psi[t,...], Y[t,...])
                h_chi[t+1,...], chi[t,:] = self.chi_target(h_chi[t,...], Y[t,...])                
        
        
        mu = mu.unsqueeze(axis=-1)    
        VaR = psi.unsqueeze(axis=-1)
        CVaR = VaR + chi.unsqueeze(axis=-1)
    ...
    def plot_summary(self):
        ...
        V = np.array(self.V)
        for t in range(self.T):
            plt.plot(V[:,t], label=r'$V_{'+str(t) + '}$', linewidth=1, alpha=0.5)
            plt.plot(self.moving_average(V[:,t],100), label=r'$V_{'+str(t) + '}$', linewidth=1)
