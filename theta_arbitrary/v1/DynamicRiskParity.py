...
class ThetaNet(nn.Module):
    ...
    def forward(self, h, Y):
        ...
        theta = 2*self.sigmoid(self.hidden_to_out(x)).squeeze()
...
class DynamicRiskParity():
    ...
    def __init__(self, Simulator : Simulator_OU, X0=1, B = 0, alpha=0.8, p=0.5):
        ...
        self.eta = 0.01 # the Lagrange multiplier factor        
    ...
    def __RunEpoch__(self, batch_size = 256):
        ...
        wealth[0,:] = torch.sum(theta[0,:,:] * S[0,:,:], axis=1)
        ...
        beta = theta * S[:-1,...] / torch.sum(theta * S[:-1,...], axis=2).unsqueeze(axis=2)
