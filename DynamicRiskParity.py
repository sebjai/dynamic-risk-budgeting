...
class DynamicRiskParity():
    ...
    def __init__(self, Simulator : Simulator_OU, X0=1, B = 0, alpha=0.8, p=0.5, test_mode = False):
        ...
        self.alpha = alpha
        self.p = p
        self.gamma = lambda u : (self.p*(u>self.alpha) + (1-self.p))/(self.p*(1-self.alpha)+(1-self.p))
    ...
    def __initialize_CVaR_VaR_Mean_Net__(self):
        ...
        self.risk_measure = lambda t, h, Y :  (self.p * self.CVaR(t, h, Y) + (1.0-self.p)* self.mu(t, h, Y)) /(self.p*(1-self.alpha) + (1.0-self.p))
        
        self.mu_target = lambda t, h, Y : self.mu_target(t, h, Y)
        ...
        self.risk_measure_target = lambda t, h, Y :  (self.p * self.CVaR_target(t, h, Y) + (1.0-self.p)* self.mu_target(t, h, Y)) / (self.p*(1-self.alpha) + (1-self.p))
    ...
    def RiskContributions(self, batch_size=2048):
        ...
        for j in range(self.T):
            ecdf = ECDF(costs_plus_V_onestep_ahead[j,:])
            U[j,:] = torch.tensor(ecdf(costs_plus_V_onestep_ahead[j,:])).float()
            Gamma[j,:] = 1.0*(U[j,:] >= self.alpha) / (1-self.alpha)
    ...
    def PlotPaths(self, batch_size=1, title=None):
        ...
        for j in range(1, self.T):
            fig = plt.figure()
            qtl = np.quantile(beta[:], [0.01, 0.99])
            
            plt.subplot(1,2,1)
            plt.title(r'$\beta_{0:1d}^1$'.format(j))
            im1=plt.scatter(S[j,:,0], S[j,:,1], s=1, alpha=0.5, c=beta[1,:,0], cmap='jet')
            ...
            im2 = plt.scatter(S[j,:,0], S[j,:,1], s=1, alpha=0.5, c=beta[1,:,1], cmap='jet')
